...
def bpsk_mod(ak, L):
    ...
    s_bb = upfirdn(
        h=[1] * L, x=2 * ak - 1, up=L
    )  # NRZ encoder (+1V for 1 and -1V for 0)
...
def qpsk_mod(a, fc, OF, enable_plot=False):
    ...
    I = a[0::2]
    Q = a[1::2]  # Evem amd odd bit streams
...
def oqpsk_demod(r, N, fc, OF, enable_plot=False):
    ...
    y = y[
        L + L // 2 - 1 : -1 - L // 2 : L
    ]  # Q arm - sample at every symbol starting at L+L/2-1th sample
...
def piBy4_dqpsk_diff_encoding(a, enable_plot=False):
    ...
    m = 2 * I + Q
    dTheta = np.array(
        [-3 * np.pi / 4, 3 * np.pi / 4, -np.pi / 4, np.pi / 4]
    )  # Lookup table for pi/4-DQPSK
    u = np.zeros(len(m) + 1)
    v = np.zeros(len(m) + 1)
    u[0] = 1
...
def msk_mod(a, fc, OF, enable_plot=False):
    ...
    ai = ak[0::2]
    aq = ak[1::2]  # Split even and odd bit streams
...
def msk_demod(r, N, fc, OF):
    ...
    Fs = OF * fc
    Ts = 1 / Fs
    Tb = OF * Ts  # Sampling frequency, durations
    ...
    u = (
        r * x * np.cos(2 * np.pi * fc * t)
    )  # Multiply I by half cosines and cos(2*pi*fc*t)
    v = (
        -r * y * np.sin(2 * np.pi * fc * t)
    )  # Multiply Q by half sines and sin(2*pi*fc*t)
...
def gmsk_mod(a, fc, L, BT, enable_plot=False):
    ...
    h = 0.5  # Modulation index (peak-to-peak frequency deviation / bit rate)
...
def bfsk_mod(a, fc, fd, L, fs, fsk_type="coherent", enable_plot=False):
    ...
    if fsk_type == "noncoherent":
        ...
        c1 = np.cos(
            2 * np.pi * (fc + fd / 2) * t + 2 * np.pi * np.random.random_sample()
        )
        ...
        c1 = np.cos(
            2 * np.pi * (fc + fd / 2) * t + phase
        )  # Carrier 1 with random phase
        c2 = np.cos(
            2 * np.pi * (fc - fd / 2) * t + phase
        )  # Carrier 2 with the same random phase
...
def bfsk_coherent_demod(r_t, phase, fc, fd, L, fs):
    ...
    x = r_t * (
        np.cos(2 * np.pi * (fc + fd / 2) * t + phase)
        - np.cos(2 * np.pi * (fc - fd / 2) * t + phase)
    )
    y = np.convolve(x, np.ones(L))  # Integrate/sum from 0 to L
    a_hat = (y[L - 1 :: L] > 0).astype(
        int
    )  # Sample at every sampling instant and detect

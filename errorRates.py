...
def qam_awgn(M,gamma_s):
    ...
    SERs = 1-(1-(1/sqrt(M))*erfc(sqrt(3/2*gamma_s/(M-1))))**2
    return SERs


def pam_awgn(M,gamma_s):
    SERs=2*(1-1/M)*.5*erfc(sqrt(3*gamma_s/(M**2-1)))
    return SERs


def integrand(q,gamma_s,M):
    return (.5*erfc((-q-np.sqrt(2*gamma_s))/np.sqrt(2)))**(M-1)*1/np.sqrt(2*pi)*np.exp(-(q**2)/2)
...
def mgf_rician(K_dB,g,gamma_s): # MGF function for Rician Channel
    ...
    fun = lambda x: ((1+K)*sin(x)**2)/((1+K)*sin(x)**2+g*gamma_s)*exp(-K*g*gamma_s/((1+K)*sin(x)**2+g*gamma_s)) # MGF Function
